  ...
  <div class="app-container">
    <el-form ref="form" :model="form" label-width="120px">
      <el-form-item label="商品类型">
        <el-select v-model="form.item_type" @change="handleOnTypeChange">
      ...
      <el-form-item label="商品ID">
        <el-select v-model="form.item_id" placeholder="请选择" @change="handleOnItemIdChange">
          <el-option
      ...
      <el-form-item label="上传图片">
        <el-upload class="avatar-uploader" :action="uplodurl" :show-file-list="false" :on-success="handleAvatarSuccess"
...
<script>
import { createItem, getListByType } from '@/api/shop'
import { getList as getSeedList } from '@/api/seed'
...
export default {
  ...
  methods: {
    ...
    handleOnTypeChange(value) {
      ...
      getSeedList(data).then(response => {
    ...
    handleOnItemIdChange(value) {
      const item = this.options.find(item => item.value === value);
      this.form.item_title = item.label;
      this.form.item_imgurl = item.imgurl;
